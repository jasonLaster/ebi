/**
 * Test file to verify P/E ratios are fetched and displayed correctly
 * 
 * Run with: bun test scripts/test-pe-ratio.test.ts
 * Or via npm script: bun run test:pe-ratio
 */
  ...
  test("API response includes peRatio field for all ETFs", async () => {
    ...
    for (const etf of etfs) {
      ...
      if (!perf.error) {
        ...
        // Note: peRatio may be null if FMP API doesn't provide it for ETFs
        if (perf.hasOwnProperty("peRatio")) {
          console.log(
          ...
          console.warn(
            `  ⚠ ${etf.toUpperCase()}: peRatio field missing from API response`
          );
          console.warn(
            `     This may indicate cached data or the API needs to be updated`
          );
          // Still fail the test to indicate the field should be present
          expect(perf).toHaveProperty("peRatio");
        }
      } else {
        console.log(`  ⚠ ${etf.toUpperCase()}: Error - ${perf.error}`);
        // If there's an error, peRatio might not be included, which is OK
      }
    }
  });
      ...
      if (matches && matches.length > 0) {
        ...
        if (potentialPeRatios.length > 0) {
          console.log(
            ...
            potentialPeRatios.slice(0, 5).map((n) => n.toFixed(2)).join(", ")
          );
        ...
        if (html.includes("N/A") || html.includes("n/a")) {
          console.log("  ℹ Found 'N/A' in HTML (P/E ratios may be unavailable)");
        }
      }
    } catch (error) {
      console.warn(
