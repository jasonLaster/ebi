...
async function extractHoldingsFromPDF(pdfPath) {
  try {
    ...
    // Save raw text for inspection (save to both locations for compatibility)
    const outputPath = path.join(__dirname, "../data/extracted_pdf_text.txt");
    const holdingsTextPath = path.join(__dirname, "../data/extracted_holdings.txt");
    fs.writeFileSync(outputPath, extractedText);
    fs.writeFileSync(holdingsTextPath, extractedText);
    console.log(`Raw text saved to: ${outputPath}`);
    console.log(`Also saved to: ${holdingsTextPath} (for parsing script)`);
...
async function main() {
  ...
  const pdfPath = args[0] || path.join(__dirname, "../data/Fund Holdings (3).pdf");

  if (!fs.existsSync(pdfPath)) {
    console.error(`PDF file not found: ${pdfPath}`);
    console.error("Usage: bun extract_pdf_holdings.js <pdf-path>");
    return;
  }
