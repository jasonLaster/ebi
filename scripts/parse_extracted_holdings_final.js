      ...
      if (currentHolding && currentHolding.ticker) {
        ...
        if (currentHolding.weight !== undefined) {
          ...
          holdings[currentHolding.ticker] = {
            name: currentHolding.company,
            weight: currentHolding.weight,
            market_value: currentHolding.marketValue,
            actual_weight: currentHolding.weight, // Using weight as actual_weight for now
...
function analyzeHoldings(holdings) {
  ...
  sortedHoldings.slice(0, 10).forEach(([ticker, data], index) => {
    console.log(
      `${index + 1}. ${ticker}: ${data.name} (${data.weight.toFixed(2)}%)`
    );
  });
...
async function main() {
  ...
  const jsonPath = saveHoldingsAsJSON(holdings, "ebi_holdings.json");
