...
async function main() {
  ...
  try {
    ...
    console.log(
      `ALL symbols in database: ${allSymbols.size} symbols`
    );
    ...
    if (irrelevantSymbols.length > 0) {
      ...
      console.log(`   Example symbols: ${irrelevantSymbols.slice(0, 10).join(", ")}`);
      console.log(
    ...
    console.log(`Target (${target}) total weight: ${(targetSum * 100).toFixed(2)}%`);
    
    for (let i = 0; i < baselines.length; i++) {
      const sum = Array.from(baselineMaps[i].values()).reduce((a, b) => a + b, 0);
      console.log(`Baseline ${baselines[i]} total weight: ${(sum * 100).toFixed(2)}%`);
    }
