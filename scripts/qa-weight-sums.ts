...
async function main() {
  ...
  try {
    ...
    for (const etf of etfs) {
      ...
      console.log(
        `  Top 10 weights: ${top10.map((w) => (w * 100).toFixed(2) + "%").join(", ")}`
      );
      ...
      const sampleEntries = Array.from(weightMap.entries())
        ...
        .slice(0, 5);
      console.log(
        `  Top 5 holdings: ${sampleEntries.map(([sym, w]) => `${sym}: ${(w * 100).toFixed(2)}%`).join(", ")}`
      );
    ...
    for (const etf of ["EBI", "VTV"]) {
      ...
      const weightSum = Array.from(weightField.values()).reduce(
        (a, b) => a + b,
        0
      );
      const actualWeightSum = Array.from(actualWeightField.values()).reduce(
        (a, b) => a + b,
        0
      );
      ...
      console.log(
        `  'actual_weight' field sum: ${(actualWeightSum * 100).toFixed(2)}%`
      );
      console.log();
    }
  } finally {
