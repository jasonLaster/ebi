...
async function createInitialTask(db: HoldingsDb): Promise<number> {
  console.debug("Creating initial task with status 'starting'.");
  await db.execute({
    sql: `INSERT INTO tasks (type, status) VALUES ('ebi', 'starting')`,
  });
  const result = await db.execute({
    sql: `SELECT id FROM tasks WHERE type = 'ebi' ORDER BY id DESC LIMIT 1`,
  });
  ...
  if (typeof taskId !== "number") {
    throw new Error("Failed to create task: invalid id");
  }
...
async function updateTaskWithRawData(
  ...
  await db.execute({
    sql: `UPDATE tasks SET raw = ?, status = 'pending' WHERE id = ?`,
    args: [JSON.stringify(rawData), taskId],
  });
...
async function updateTaskSuccess(
  ...
  await db.execute({
    sql: `UPDATE tasks SET status = 'successful', data = ? WHERE id = ?`,
    args: [JSON.stringify(data), taskId],
  });
...
async function updateTaskFailure(
  ...
  await db.execute({
    sql: `UPDATE tasks SET status = 'failed', error = ?, raw = ?, data = NULL WHERE id = ?`,
    args: [error.message, rawJson, taskId],
  });
...
export async function getEBIDetails(): Promise<EBIResult | null> {
  ...
  try {
    ...
    taskId = await createInitialTask(db); // Create task first
    ...
    await updateTaskWithRawData(db, taskId, result);
    ...
    await updateTaskSuccess(db, taskId, finalResult);
    return finalResult;
  } catch (error) {
    console.error("Overall error in getEBIDetails", error);
    if (taskId !== undefined) {
      await updateTaskFailure(db, taskId, error as Error, result);
    }
